...
- **占位符命名语义规则**：找到语义词汇确定内容类型，解析技术参数并应用限制
  * `{bullet_2}` - 重点是bullet（要点内容），数字2表示第2个要点
  * `{bullet_2_time_1}` - 重点是time（时间段），表示第2个要点下的第1个时间段
  * `{title_max_token_50}` - 重点是title（标题），max_token_50表示内容限制为50个token
  * `{section_3_title}` - 重点是title（标题），表示第3个部分的标题
- **Token限制处理规则**：当占位符名称包含max_token_N时，严格控制内容长度
  * 如果内容超过N个token，截断到N个token范围内，末尾添加"..."表示内容被截断
  * 优先保留内容的开头部分，确保核心信息不丢失
  * 示例：max_token_20限制下，21个token的内容变为"前17个token的内容..."
- **内容类型匹配**：根据语义词汇确定填充内容类型
...
- action: "replace_placeholder"
- placeholder: 占位符名称
- content: 优化后的填充内容（如有token限制，内容被截断时末尾加"..."）
- reason: 选择理由
  ...
  "assignments": [
    {
      "slide_index": 0,
      "action": "replace_placeholder", 
      "placeholder": "title_max_token_10",
      "content": "数字化转型项目...",
      "reason": "标题内容超过10个token限制，截断并添加省略号"
    }
  ]
}
```
